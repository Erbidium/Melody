...
public class Recommender : IRecommender
{
    ...
    public async Task<Result<List<long>>> GetRecommendedSongsIds(RecommendationsPreferences recommendationsPreferences,
        int page = 1, int pageSize = 10)
    {
        ...
        if (recommendationsPreferences.StartYear.HasValue)
            queryContainers.Add(
                descriptor.SearchSongsReleasedLaterThanYear(recommendationsPreferences.StartYear.Value));
        if (recommendationsPreferences.EndYear.HasValue)
            queryContainers.Add(
                descriptor.SearchSongsReleasedEarlierThanYear(recommendationsPreferences.EndYear.Value));
